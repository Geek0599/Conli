(() => {
    ...
    function gallerySlider() {
        const sliderBlock = document.querySelector('[data-slider="gallery"]');
        if (sliderBlock) {
            const slider = sliderBlock.querySelector(`[data-slider]`);
            const sliderPagination = sliderBlock.querySelector("[data-slider-pagination]");
            ...
            if (slider) {
                const swiper = new Swiper(slider, {
                    ...
                    slidesPerView: 2,
                    spaceBetween: 4,
                    initialSlide: Math.floor(allSildes.length / 2) - 1,
                    speed: 800,
                    lazy: true,
                    pagination: {
                        el: sliderPagination,
                    ...
                    on: {
                        click: () => {
                            ...
                            const activeIndex = swiper.activeIndex;
                            if (clickedIndex !== void 0 && clickedIndex !== activeIndex) swiper.slideTo(clickedIndex);
                        }
                    }
                });
            }
    ...
    function toggleBtn() {
        ...
        const toggleElems = document.querySelectorAll("[data-toggle-content]");
        if (toggleBtns.length) document.addEventListener("click", (e => {
            const target = e.target;
            if (target.closest("[data-toggle]")) {
                const toggle = target.closest("[data-toggle]");
                toggle.classList.toggle("_active");
                toggle.parentElement.querySelector("[data-toggle-content]") ? toggle.parentElement.querySelector("[data-toggle-content]").classList.toggle("_open") : null;
            } else if (!target.closest("[data-toggle]") && !target.closest("[data-toggle-content]")) {
                toggleElems.forEach((elem => elem.classList.remove("_open")));
                toggleBtns.forEach((elem => elem.classList.remove("_active")));
            }
        }));
    }
    function selectValuePlaceholder() {
    ...
    function formValidate() {
        ...
        async function checkInputs(inputs, form, event) {
            ...
            if (!errors) onSuccesFormValidate(form);
        }
        async function checkInput(input) {
            ...
            if (input.required) {
                ...
                if (input.type === "checkbox" && input.checked === false) isError = true;
    ...
    formValidate();
    function onSuccesFormValidate(form) {
        const recaptchaField = form.querySelector("#g-recaptcha");
        if (recaptchaField) grecaptcha.execute();
        console.log(Object.fromEntries(new FormData(form)));
    }
    function setupRecaptchaHandler(form, recaptchaField) {
        recaptchaField.addEventListener("change", (async function() {
            console.log("RECAPTCHA FIELD CHANGE");
